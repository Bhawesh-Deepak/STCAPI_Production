    ...
    public class QlikDataAccessAPI : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateDataAccess(QlikDataAccess model)
        {
            ...
            if (filteredModel.TEntities.Any())
            {
                ...
                var deleteResponse = await _IQlickDataAccessRepository.DeleteEntities(filteredModel.TEntities.ToArray());
            }

            var response = await _IQlickDataAccessRepository.CreateEntity(model);
            return Ok(response);
        }
